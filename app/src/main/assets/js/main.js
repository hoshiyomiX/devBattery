...
(function() {
    ...
    function spawnBubble(container) {
        ...
        const size = Math.random() * 6 + 3;  // 3-9px - smaller for smoother motion
        const left = Math.random() * 85 + 7.5;
        // More natural duration variation based on size
        const sizeFactor = 1.0 - ((size - 3) / 6) * 0.3; // smaller bubbles rise faster
        const baseDuration = 2.2;
        const duration = (baseDuration * sizeFactor) + (Math.random() * 0.6); // 1.5-2.8s
        bubble.style.width = size + 'px';
        bubble.style.height = size + 'px';
        bubble.style.left = left + '%';
        bubble.style.animationDuration = duration + 's';
        bubble.style.animationDelay = (Math.random() * 0.1) + 's'; // slight delay for natural stagger
